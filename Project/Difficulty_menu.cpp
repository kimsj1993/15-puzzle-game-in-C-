...
Difficulty_menu::Difficulty_menu(Point xy, int w, int h, const string& title)
        :Window{ xy,w,h,title },
        background{ Point{0, 0}, Point{x_max(), y_max()} },
        difficulty_text{ Point{ 25, 40 }, "Select a difficulty:" },
        easy_button{ Point{ x_max() / 2 - 304, y_max() / 2 }, 140, 40, "Easy (10 Moves)", [](Address, Address pw) {reference_to<Difficulty_menu>(pw).change_state(1); } },
        normal_button{ Point{ x_max() / 2 - 148, y_max() / 2 }, 140, 40, "Normal (20 Moves)", [](Address, Address pw) {reference_to<Difficulty_menu>(pw).change_state(2); } },
        hard_button{ Point{ x_max() / 2 + 8, y_max() / 2}, 140, 40, "Hard (40 Moves)", [](Address, Address pw) {reference_to<Difficulty_menu>(pw).change_state(3); } },
        expert_button{ Point{ x_max() / 2 + 164, y_max() / 2}, 140, 40, "Expert (80 Moves)", [](Address, Address pw) {reference_to<Difficulty_menu>(pw).change_state(4); } },
...
void Difficulty_menu::stylize_objects() {
        background.set_fill_color(Color::white);
        difficulty_text.set_font_size(35);
}

void Difficulty_menu::attach_objects() {
        attach(background);
...
void Difficulty_menu::quit() {
        hide();
}

int Difficulty_menu::change_state(int choice) {
        button_pushed = true;
        selection = choice;
        return choice;
}

void Difficulty_menu::wait_for_button() {
        show();
        while (!button_pushed) {
                Fl::wait();
        }
