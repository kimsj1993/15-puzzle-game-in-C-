...
Difficulty_window::Difficulty_window(Point xy, int w, int h, const string& title)
	...
	easy_button{ Point{ x_max() / 2 - 304, y_max() / 2 }, 140, 40, "Easy (10 Moves)",
		[](Address, Address pw) {reference_to<Difficulty_window>(pw).change_state(1); } },
	normal_button{ Point{ x_max() / 2 - 148, y_max() / 2 }, 140, 40, "Normal (20 Moves)", 
		[](Address, Address pw) {reference_to<Difficulty_window>(pw).change_state(2); } },
	hard_button{ Point{ x_max() / 2 + 8, y_max() / 2}, 140, 40, "Hard (40 Moves)", 
		[](Address, Address pw) {reference_to<Difficulty_window>(pw).change_state(3); } },
	expert_button{ Point{ x_max() / 2 + 164, y_max() / 2}, 140, 40, "Expert (80 Moves)", 
		[](Address, Address pw) {reference_to<Difficulty_window>(pw).change_state(4); } }, // C++11 features (lambda expressions)
...
void Difficulty_window::attach_objects() { // Attaches objects
	attach(background);
...
void Difficulty_window::quit() { // Exits window
	hide();
}

int Difficulty_window::change_state(int choice) { 
	// Detects button press and returns selection
	button_pushed = true;
	selection = choice;
	return choice;
}

void Difficulty_window::wait_for_button() { // Waits for button press
	show();
	while (!button_pushed) {
		Fl::wait();
	}
