...
Main_window::Main_window(Point xy, int w, int h, const string& title)
	...
	start_button{ Point{ x_max() / 2 - 70, y_max() / 2 }, 140, 40, "Play", [](Address, Address pw) {reference_to<Main_window>(pw).change_state(1); } },
	help_button{ Point{ x_max() / 2 - 35, y_max() / 2 + 80 }, 70, 20, "Help", [](Address, Address pw) {reference_to<Main_window>(pw).change_state(2); } },
	quit_button{ Point{ x_max() / 2 - 35, y_max() / 2 + 140 }, 70, 20, "Quit", [](Address, Address pw) {reference_to<Main_window>(pw).change_state(0); } },
